---
title: "SOA Predictive Analytics Exam, Set up"

---

Run CHUNK 1 to load the Breast Cancer dataset and display the first few rows. Before doing so, you must ensure that the working directory is the one where you saved the dataset. To change the working directory, go to Session>Set Working Directory.


```{r}
# CHUNK 1
# Load the data. This command produces no output
data.all <- read.csv("breastcancerwisconsindataset.csv", stringsAsFactors = TRUE)
# Display the first six rows of the data set. They should appear below.
head(data.all)
```  

Run the code in CHUNK 2 to construct a decision tree. 
 
```{r}
# CHUNK 2
# Load the two needed libraries
library(rpart)
library(rpart.plot)

# Set a seed. This is to ensure that if random numbers are being used the output will be the same every time. It is not always obvious if a package uses random numbers.
set.seed(1234)

# Construct a decision tree to predict the variable "diagnosis." First, list the variables to be used as predictors.
formula <- as.formula("diagnosis~radius_mean+texture_mean+perimeter_mean+area_mean+smoothness_mean+compactness_mean+concavity_mean")

# Use the rpart package to build a classification tree
tree <- rpart(formula, data = data.all, cp = .009)

# Use the rpart.plot package to display the tree.
rpart.plot(tree, type = 0, digits = 4)
```
